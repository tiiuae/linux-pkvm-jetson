...
/*
 * Page table allocator wrapper for ARM SMMUv2 pKVM
 *
...
void *__arm_lpae_alloc_data(struct io_pgtable_cfg *cfg, size_t size, gfp_t gfp)
{
	...
	if (size > PAGE_SIZE) {
		hyp_err("__arm_lpae_alloc_data: size %zu > PAGE_SIZE", size);
		return NULL;
	}
	...
	if (cfg->quirks & IO_PGTABLE_QUIRK_IDMAP) {
		addr = kvm_iommu_donate_pages_atomic(get_order(size));
		if (!addr)
			hyp_err("kvm_iommu_donate_pages_atomic failed (order=%u)", get_order(size));
		return addr;
	}
