...
#ifndef __SMMU_PLATFORM_H
#define __SMMU_PLATFORM_H
...
/**
 * struct smmu_platform_hooks - Platform-specific callbacks for SMMUv2 pKVM
 *
 * @init: Called during smmu_global_init() after SMMU instances are
 *        initialized. Platform code can use this to initialize its own
 *        MMIO regions, register callbacks, etc. Return 0 on success.
 *
 * @mmio_handler: Called from smmu_dabt_handler() when an MMIO access
...
/**
 * smmu_register_platform_hooks - Register platform-specific callbacks
...
/**
 * smmu_lookup_sid - Look up SID assignment information
